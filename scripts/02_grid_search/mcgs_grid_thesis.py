...
EXPERIMENT_NAME = "mcgs-grid-thesis"
...
for rollouts in ROLLOUT_VALUES:
    ...
    job_name = f"mcgs-grid-r{rollouts}-e{num_evals}"
    ...
    cmd = (
        ...
        f"desired_length=30 "
        f"batch_size=64 "
        f"logger.run_name='mcgs_r_{rollouts}_e_{num_evals}_c_${{method.exploration_c}}_a_${{method.alpha}}' "
        f"-m"
    )
